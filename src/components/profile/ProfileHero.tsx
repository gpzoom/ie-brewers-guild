...
type ProfileHeroProps = {
  ...
  // row's own), for the "In these locations" line -- see
  // member-profile.server.ts's locationCities doc comment. Length 1 for a
 ...
 */
export function ProfileHero({ member, coverUrl, logoUrl, locationCities }: ProfileHeroProps) {
    ...
    <div className="relative">
      ...
      <div className={heroRowClassName}>
        ...
        <div className="flex flex-1 flex-col pb-1">
          ...
          {/* Until the Guild decides whether each location gets its own
              profile page, a multi-location business has exactly ONE
              profile (the first-imported location's row) and every other
              location is plain text here, not a link -- see
              member-profile.server.ts's locationCities doc comment. */}
          <p className="text-sm text-ink-muted">
            {locationCities.length > 1
              ? `In these locations: ${locationCities.join(" • ")}`
