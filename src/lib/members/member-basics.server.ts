...
export const updateMemberBasics = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (patch.logo_background !== undefined && !isLogoBackground(patch.logo_background)) {
...
export const getMemberBasics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: member, error } = await supabase
      ...
      .select(
        "id, business_name, tagline, city, state, street_address, service_area, lead_time, member_since_year, timezone, member_type, logo_background, theme, discount_percent, discount_no_fixed_percent, discount_redeem_text",
      )
      .eq("id", data.memberId)
      .single();
