...
export const submitCreatorUpload = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: formData }) => {
    ...
    // `cloudflare:workers` resolves reliably here because this code only
    // ever runs inside a createServerFn handler (same pattern as
    ...
    // env`-block gotcha already, see project_staging_environment.md), this
    // must NOT silently allow every request through with no limiting and
    // no signal that anything is wrong. This is the sole security boundary
    // on the most exposed endpoint in this plan (fully public, no auth),
    // so a missing binding is treated as a hard failure, loudly logged,
    // rather than defaulting to `{ success: true }`.
    ...
    try {
      ...
      // parse (its own doc comment: never fall back to unstripped bytes).
      // That's a real, if rare, user-reachable case (e.g. a Live
      // Photo/Motion Photo JPEG) -- not an internal failure -- but its
      // thrown message is written for a developer, not an anonymous
      // caller, so it's logged here and replaced with a plain one.
      console.error("submitCreatorUpload: stripImageMetadata rejected the file", err);
      throw new Error("This file couldn't be processed. Please upload a JPEG or PNG photo.");
    }
    ...
    // deliberately racing requests) could both read the same used_count
    // above, both pass the checks above, both upload, and both increment
    // -- letting a token accept more files than max_files. The
    // `.eq("used_count", token.used_count)` clause makes the UPDATE itself
    // the check: it only matches (and therefore only returns a row) if
    // used_count still equals the value this request read; if a
    ...
    // this plan): if the upload/insert work below fails AFTER this
    // reservation succeeds, used_count has already been incremented with
    // no corresponding asset. That's a rare edge case (a storage/DB
    // failure, not a normal path) and isn't worth a rollback/give-back
    // mechanism. Moving file validation above this point (see comment
    // above) already closes the much more common case -- an invalid file
    // burning a slot -- essentially for free.
